...
def test_pod_creation_and_retrieval():
    ...
    with mock_kubernetes() as mock_k8s:
        ...
        # Retrieve the pod
        retrieved_pod = core_api.read_namespaced_pod(name="test-pod", namespace="default")
...
def test_pod_listing_with_labels():
    ...
    with mock_kubernetes() as mock_k8s:
        ...
        web_pods = core_api.list_namespaced_pod(namespace="default", label_selector="app=web")
    ...
    with mock_kubernetes() as mock_k8s:
        ...
        deployment = k8s_client.V1Deployment(
            metadata=k8s_client.V1ObjectMeta(
                ...
                labels={"runtime_id": "test-runtime"}
            ),
        ...
        pods = core_api.list_namespaced_pod(namespace="default", label_selector="runtime_id=test-runtime")
...
def test_owner_reference_cascading_deletion():
    ...
    with mock_kubernetes() as mock_k8s:
        ...
        # Verify pod exists
        pods_before = core_api.list_namespaced_pod(namespace="default", label_selector="runtime_id=test-runtime")
        assert len(pods_before.items) == 1
        
        # Delete deployment
        apps_api.delete_namespaced_deployment(name="test-deployment", namespace="default")
        
        # Verify pods are also deleted (cascading deletion)
        pods_after = core_api.list_namespaced_pod(namespace="default", label_selector="runtime_id=test-runtime")
