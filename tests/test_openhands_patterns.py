                    ...
                    spec=k8s_client.V1PodSpec(
                        ...
                        containers=[
                            k8s_client.V1Container(
                                ...
                                env=[
                                    k8s_client.V1EnvVar(name="RUNTIME_ID", value=runtime_id),
                                    k8s_client.V1EnvVar(name="SESSION_API_KEY", value="test-key")
                                ],
        ...
        service_manifest = k8s_client.V1Service(
            ...
            spec=k8s_client.V1ServiceSpec(
                ...
                ports=[
                    ...
                    k8s_client.V1ServicePort(name="vscode", port=8000, target_port=8000),
                    k8s_client.V1ServicePort(name="work1", port=12000, target_port=12000),
                    k8s_client.V1ServicePort(name="work2", port=12001, target_port=12001),
                ]
            )
        )
                                ...
                                k8s_client.V1HTTPIngressPath(
                                    ...
                                    backend=k8s_client.V1IngressBackend(
                                        service=k8s_client.V1IngressServiceBackend(
                                            name=f"runtime-{runtime_id}",
                                            port=k8s_client.V1ServiceBackendPort(number=8000)
                                        )
                                    )
                                )
                            ]
...
def test_pod_status_monitoring_pattern():
    ...
    with mock_kubernetes() as mock_k8s:
        ...
        deployment = k8s_client.V1Deployment(
            ...
            spec=k8s_client.V1DeploymentSpec(
                replicas=1,
                selector=k8s_client.V1LabelSelector(match_labels={'runtime_id': runtime_id}),
                template=k8s_client.V1PodTemplateSpec(
...
def test_cascading_deletion_pattern():
    ...
    with mock_kubernetes() as mock_k8s:
        ...
        deployment = k8s_client.V1Deployment(
            ...
            spec=k8s_client.V1DeploymentSpec(
                ...
                template=k8s_client.V1PodTemplateSpec(
                    metadata=k8s_client.V1ObjectMeta(labels={'runtime_id': runtime_id}),
                    spec=k8s_client.V1PodSpec(
                        containers=[k8s_client.V1Container(name="runtime", image="test")]
                    )
                )
            )
        )
        
        created_deployment = apps_api.create_namespaced_deployment(namespace=namespace, body=deployment)
        ...
        pvc = k8s_client.V1PersistentVolumeClaim(
            ...
            spec=k8s_client.V1PersistentVolumeClaimSpec(
                access_modes=["ReadWriteOnce"],
                resources=k8s_client.V1ResourceRequirements(requests={"storage": "1Gi"})
            )
        )
        ...
        assert core_api.read_namespaced_persistent_volume_claim(f"runtime-{runtime_id}", namespace)
        assert policy_api.read_namespaced_pod_disruption_budget(f"runtime-{runtime_id}", namespace)
        
        pods_before = core_api.list_namespaced_pod(namespace=namespace, label_selector=f'runtime_id={runtime_id}')
        assert len(pods_before.items) == 1
        
        # Delete deployment (should cascade to all dependent resources)
        apps_api.delete_namespaced_deployment(name=f"runtime-{runtime_id}", namespace=namespace)
        ...
        with pytest.raises(ApiException) as exc:
            core_api.read_namespaced_persistent_volume_claim(f"runtime-{runtime_id}", namespace)
        assert exc.value.status == 404
        
        with pytest.raises(ApiException) as exc:
            policy_api.read_namespaced_pod_disruption_budget(f"runtime-{runtime_id}", namespace)
        assert exc.value.status == 404
        
        # Verify pods are also deleted
        pods_after = core_api.list_namespaced_pod(namespace=namespace, label_selector=f'runtime_id={runtime_id}')
