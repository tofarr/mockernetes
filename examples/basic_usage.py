    ...
    with mock_kubernetes() as mock_k8s:
        ...
        pod = k8s_client.V1Pod(
            metadata=k8s_client.V1ObjectMeta(
                name="example-pod", labels={"app": "example"}
            ),
        ...
        app_pods = core_api.list_namespaced_pod(
            namespace="default", label_selector="app=example"
        )
        print(f"Pods with app=example: {len(app_pods.items)}")

        # Get pod logs
        logs = core_api.read_namespaced_pod_log(
            name="example-pod", namespace="default", container="web"
        )
...
def example_deployment_with_controller_simulation():
    ...
    with mock_kubernetes() as mock_k8s:
        ...
        deployment = k8s_client.V1Deployment(
            metadata=k8s_client.V1ObjectMeta(
                name="web-deployment", labels={"app": "web"}
            ),
            spec=k8s_client.V1DeploymentSpec(
                ...
                template=k8s_client.V1PodTemplateSpec(
                    ...
                    spec=k8s_client.V1PodSpec(
                        containers=[
                            k8s_client.V1Container(name="web", image="nginx:latest")
                        ]
                    ),
                ),
            ),
