        ...
        'pods': [
            {
                ...
                'spec': {
                    'containers': [{'name': 'app', 'image': 'nginx:1.20'}]
                }
            }
        ]
    }
...
def test_deployment_scaling(replica_count, k8s_cluster):
    ...
    pods = core_api.list_namespaced_pod(namespace="default", label_selector="app=scalable")
...
class TestRuntimeLifecycle:
    ...
    def test_runtime_creation(self):
        ...
        self.core_api.create_namespaced_service_account(namespace=self.namespace, body=sa)
        ...
        deployment = k8s_client.V1Deployment(
            ...
            spec=k8s_client.V1DeploymentSpec(
                replicas=1,
                selector=k8s_client.V1LabelSelector(match_labels={'runtime_id': self.runtime_id}),
                template=k8s_client.V1PodTemplateSpec(
                    metadata=k8s_client.V1ObjectMeta(labels={'runtime_id': self.runtime_id}),
                    spec=k8s_client.V1PodSpec(
        ...
        pods = self.core_api.list_namespaced_pod(
            namespace=self.namespace,
            label_selector=f'runtime_id={self.runtime_id}'
        )
    ...
    def test_runtime_with_dependencies(self):
        ...
        assert self.core_api.read_namespaced_service(f"runtime-{self.runtime_id}", self.namespace)
        assert self.core_api.read_namespaced_persistent_volume_claim(f"runtime-{self.runtime_id}", self.namespace)
        ...
        with pytest.raises(ApiException):
            self.core_api.read_namespaced_service(f"runtime-{self.runtime_id}", self.namespace)
        
        with pytest.raises(ApiException):
            self.core_api.read_namespaced_persistent_volume_claim(f"runtime-{self.runtime_id}", self.namespace)
